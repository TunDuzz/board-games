...
exports.makeAiMove = async (req, res) => {
  try {
    ...
    if (gameType === "caro") {
        ...
        const moveResult = getBestCaroMove(boardState, botRole === "player1" ? "Black" : "White", difficulty);
        return res.json({
        ...
        const moveResult = getBestChessMove(boardState, botRole === "player1" ? "white" : "black", difficulty);
        return res.json({
        ...
        const moveResult = getBestXiangqiMove(boardState, botRole === "player1" ? "red" : "black", difficulty);
        return res.json({
