...
const XiangqiGame = () => {
            ...
            if (match.moves && match.moves.length > 0) {
                ...
                match.moves.forEach(m => {
                    ...
                    if (moveData && moveData.from && moveData.to) {
                        ...
                        const desc = `${piece?.label || "Quân"} (${moveData.from.col + 1},${moveData.from.row + 1}) ➔ (${moveData.to.col + 1},${moveData.to.row + 1})`;
                        newHistory.push({
                            piece,
                            from: moveData.from,
                            to: moveData.to,
                ...
                // Xiangqi: Red (player1) goes first
        ...
        try {
          ...
          if (data.matched) {
            ...
            if (data.room?.room_id) {
               navigate(`/game/xiangqi?roomId=${data.room.room_id}&code=${data.room.room_code}`);
            }
          }
        } catch (error) {
          clearInterval(interval);
  ...
  }, [turn, mode, isAiThinking, isGameOver, difficulty]);
  ...
  const executeMove = (fromRow, fromCol, toRow, toCol, shouldEmit = true) => {
    ...
    if (capturedPiece) {
        if (turn === 'red') setRedCaptures(prev => prev + 1);
    ...
    if (capturedGeneral) {
        ...
        if (shouldEmit && roomId) {
            socket.emit("game_over", { roomId, matchId, result: "win", winnerId: myUserId, duration: Math.floor((Date.now() - gameStartTime) / 1000), p1Captures: redCaptures, p2Captures: blackCaptures });
        }
    } else {
        ...
        if (isCheckmate(newBoard, nextTurn)) {
            ...
            if (isInCheck(newBoard, nextTurn)) {
                sonnerToast.warning(`Chiếu tướng! Quân ${nextTurn === 'red' ? 'Đỏ' : 'Đen'} đang bị chiếu.`);
            }
            if (targetPiece) {
                playSfx('capture');
            } else {
  ...
  const handleResign = () => {
    if (window.confirm("Bạn có chắc chắn muốn đầu hàng?")) {
      const duration = Math.floor((Date.now() - gameStartTime) / 1000);
      const p1Caps = redCaptures;
      const p2Caps = blackCaptures;
  ...
  const handleStartGame = async () => {
    if (playerCount >= 2) {
      ...
      try {
        await matchmakingService.joinQueue("xiangqi");
        setIsSearching(true);
